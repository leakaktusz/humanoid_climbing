...
sys.path.insert(0, os.path.join(_REPO_ROOT, "climbing_env_mjx", "climbing_env_mjx"))
...
def _load_agent(checkpoint: Path, env, seed: int) -> sac.SAC:
...
def main(args: Args) -> None:
    print(f"[INIT] Loading '{args.env}' …")
    env = climbing_mjx.load(environment_name=args.env)
    ...
    if args.random:
        ...
        print(f"[EVAL] Mode: random actions (no checkpoint) — visualizing the env\n")
    else:
    ...
    def get_action(obs_jax: jnp.ndarray) -> np.ndarray:
        ...
        if args.stochastic:
            ...
            action = np.asarray(dist.sample(seed=key))
            print(f"action={action[:4].round(3)}  std={np.std(action):.4f}")
            return action
        else:
